(function () {
  ...
  const dimensions = [
    ...
    },
    {
      ...
      questions: [
        ...
        ['寒暄、客套或没有明确目的的闲聊会消耗我很多注意和准备。', '观察非任务型交流中的规则推断与回应成本。', '例如见面时知道应该聊几句，却一直在想该问什么、何时结束。'],
        ['多人快速交谈时，我可能能听见每个人的话，却很难同时跟踪话题和人物立场。', '观察群体交流中多来源社会信息的实时整合。', '例如讨论转了几个来回后，我不确定当前是谁在回应谁。'],
  ...
  function renderSuggestions(ranked) {
    const target = document.getElementById('personalizedSuggestions');
    const actions = buildReflectionActions(ranked);
    target.innerHTML = `
      <h3 class="result-section-heading">深度反思与行动建议</h3>
      <section class="insight-card nd-reflection-actions">
