(function () {
  ...
  const dimensions = [
    {
      ...
      questions: [
        ...
        ['某些光线、衣物材质、触碰、气味或温度会让我明显不适，甚至需要立刻离开。', '观察不同感官通道的高敏感体验。', '例如荧光灯、衣领标签或香水让我难以继续手头活动。'],
        ...
        ['同时出现多种感官输入时，我会很快疲惫、烦躁或难以组织行动。', '观察多通道刺激叠加后的整体负荷。', '例如商场里的灯光、音乐、人群和气味叠加后，让我难以继续购物。'],
    ...
    },
    {
      ...
      questions: [
        ...
        ['同时处理多个步骤、优先级或规则时，我容易卡住、漏项或反复确认。', '观察排序、切换和多步骤协调负荷。', '例如需要比较多个条件的行政流程让我迟迟无法提交。'],
    ...
    },
    {
      ...
      questions: [
        ...
        ['我倾向于按字面或精确定义理解语言，模糊表达容易让我困惑。', '观察语言精确度需求。', '例如“等会儿”“差不多”会让我不知道该如何安排。'],
        ...
        ['如果别人只说“您应该知道”而不明确指出问题，我通常很难据此调整。', '观察社会反馈是否需要具体、可操作的说明。', '例如被告知“表现自然一点”时，我不知道究竟要改变音量、眼神还是措辞。'],
    ...
    },
    {
      ...
      questions: [
        ...
        ['声音、社交、决策和任务累积到一定程度后，我会突然无法继续承受。', '观察多种负荷累积形成的过载。', '例如白天每件事都能应付，最后一个小要求却让我崩溃。'],
  ...
  const dimensionExplanationNotes = {
    ...
    transition: '判断重点是变化、切换和不确定性在准备、重新进入与恢复上带来的消耗，而不是是否愿意尝试新事物。',
  ...
  const dimensionReportMeta = {
    sensory: {
      visible: '您可能更容易注意到某些声音、光线、触感或身体信号，也可能在部分通道上出现低察觉、延迟察觉或明显的刺激寻求。关键不是“敏感”或“迟钝”二选一，而是不同刺激在不同状态下可能呈现不对称阈值。',
    ...
    attention: {
      visible: '您的注意可能更受兴趣、新奇、紧迫感、即时反馈和环境刺激调节。可能同时存在容易分心与高度专注，这两者并不矛盾：问题往往在于注意是否能按意愿分配和切换。',
      ...
      reflection: '您的注意最容易被什么点燃、被什么打断？外部结构改变后，表现能改善到什么程度？',
    ...
    executive: {
      visible: '您可能知道目标是什么，却在任务分解、工作记忆、时间估计、优先级、收尾或未来意图唤起上需要额外支架。能制定计划与能持续使用计划并不是同一件事。',
      cost: '大量小任务和小决定会持续占用管理资源；问题常出现在“开始之前”“切换之间”和“完成之后”这些边界，而不一定出现在任务核心能力本身。',
      support: '把记忆、顺序、截止时间和完成标准外置到可见系统，并降低系统本身的维护成本。最有效的工具通常是能在正确时点主动提醒您的工具。',
      reflection: '您最常卡在启动、排序、时间感、记忆、收尾还是持续维护？哪一种外部支架能稳定改善，而不是只短期新鲜？',
    ...
    communication: {
      visible: '您可能更依赖直接、明确、可操作的信息，并在即时口头交流、暗示整合、多人节奏或快速组织表达时承担额外加工步骤。文字或延迟回应可能明显提高表达质量。',
      cost: '持续猜测潜台词、监控接话时机和组织非语言表现会占用工作记忆；当信息来源多或情绪复杂时，误解和延迟反应更容易发生。',
      support: '把“猜”改成“说清楚”：要求具体说明、允许复述确认、重要事项保留文字记录，并为复杂交流留出延迟处理时间。',
      reflection: '您在哪些沟通媒介中最准确？困难主要来自暗示、速度、多人信息、即时表达，还是被误读后的修正成本？',
    ...
    transition: {
      visible: '您可能依赖可预测性、明确结束点和熟悉路径来降低切换成本。计划变化、被突然打断、陌生流程或选项过多，都可能让大脑需要额外时间重新建立行动路径。',
    ...
    activity: {
      visible: '您的活动水平和回应速度可能随唤醒状态明显变化，可能需要走动、摆弄物品或额外刺激来维持专注，也可能在兴奋、焦虑或赶时间时更难暂停和复核。',
      cost: '高唤醒时容易出现插话、快速答应、立即搜索或同时启动多个任务；低反馈和长等待则可能造成明显焦躁或注意漂移。',
      support: '把安全活动纳入任务，而不是强行压制；对高后果决定设置外部暂停点，例如延迟发送、二次检查或让他人复核。',
      reflection: '哪些动作是在帮助您调节，哪些冲动会真正带来后果？怎样设计“允许动”和“必须停一下”的边界？',
    ...
    overload: {
      visible: '您的负荷可能以累积方式上升：单个刺激、任务或交流都还能处理，但叠加后会出现思考、语言、选择和行动能力的暂时下降。恢复速度也可能明显慢于外界预期。',
      ...
      reflection: '您的最早预警是什么？头痛、说话减少、错误增加、声音变刺耳、动作变快，还是别的信号？',
    ...
    masking: {
      visible: '您可能通过模仿、脚本、自我监控、隐藏调节动作或压下支持需要来维持符合环境期待的表现。外在“看起来没问题”与内部成本可以同时存在。',
      ...
      reflection: '哪些调整是您主动选择的有效策略，哪些主要是为了避免被评价？停止某种掩饰后，功能和安全感分别会怎样变化？',
      context: { work: '表面稳定不代表低负荷；长期维持“自然、专注、好相处”的表现可能让下班后的功能显著下降。', social: '脚本、模仿和礼貌表演可能帮助进入互动，却也可能让您很难判断自己是否真正享受或同意。', daily: '在公共空间压住动作、感官需要或困惑，往往会把成本转移到回家后的反弹式安静、活动或情绪释放。', recovery: '恢复不仅是从社交活动恢复，也可能是从长期自我监控中退出；安全环境中的“变得不一样”不等于故意反差。' }
    }
  };
  ...
  const pairInterpretations = {
    ...
    'masking|sensory': '如果您经常隐藏感官不适或调节动作，外界看到的耐受度可能高于真实耐受度，成本则在活动结束后集中显现。',
    'attention|executive': '注意启动、维持和转移与执行功能会形成明显耦合：不是“不知道该做什么”，而是知道目标后仍难以把注意稳定地放到正确步骤并持续到收尾。',
    'attention|communication': '实时对话需要持续注意和快速整合信息，分心或高度聚焦都可能让您错过话题变化；文字沟通常因允许重读和延迟处理而更稳定。',
    ...
    'attention|masking': '社交中的自我监控会占用本就有限的注意资源，使您更难同时跟踪内容、身体状态和环境刺激；表面配合可能以更高的后续疲劳为代价。',
    ...
    'communication|transition': '互动规则、话题或安排突然变化时，您既要重新理解情境，又要迅速组织回应，因此临时变动在社交场景中可能比单纯改计划更耗能。',
    ...
    'communication|masking': '您可能用脚本、模仿和自我监控弥补即时社会信息加工困难；这些策略可能有效，但也会显著增加每次互动的能量成本。',
    ...
    'activity|masking': '如果您长期压住走动、重复动作或自然节奏，外表可能更安静，但调节需求并未消失，往往会转化为更高的内在紧张和活动后的反弹。',
  ...
  function getProfilePattern(ranked) {
    ...
    if (notable.length >= 1 && spread >= 1.5) {
      return {
        label: '局部突出画像',
        text: '您的维度差异较明显，负荷更集中在少数领域。可以优先针对高分维度做具体适配，同时保留对低分维度在特殊情境下变化的观察。'
      };
    }
  ...
  function renderTypeJudgment(ranked) {
    ...
    target.innerHTML = `<p class="result-lead">您的总体轮廓以<strong>${first.title}</strong>为相对突出方向，<strong>${second.title}</strong>次之；<strong>${lowest.title}</strong>相对收敛。维度极差为 <strong>${spread.toFixed(1)}</strong> 分，呈现${structure}。</p>`;
  }
