...
- PrismSelf 已进入稳定维护状态，功能和内容已基本定型。
- 本项目不再承担新内容开发、架构演进或大规模内容扩展任务。
- 后续维护限于缺陷修复、兼容性调整、必要的文档修正和现有内容更新。
- 除非用户明确要求新增内容或功能，否则默认把当前实现视为最终形态，不主动提出内容扩展、技术栈升级或"顺便优化"。
...
1. 用户当前明确要求。
2. 当前仓库的 HTML、CSS、JavaScript 源码和静态资源。
3. 本文件记录的实现约束和历史决定。
4. README 等面向用户的说明文字。
